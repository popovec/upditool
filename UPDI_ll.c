...
updi_send_break ()
{
  ...
  WriteFile (hComm, b, 2, &dNoOfBytesWritten, NULL);

  // check return code from WriteFile ? or this is enough ?
  if (2 != dNoOfBytesWritten)
    return 1;
...
int
updi_transaction (uint8_t * send, uint16_t s_len, uint8_t * receive,
		  uint16_t r_len, int timeout)
{
...
// 0 = OK
static int
updi_serial_rx_clear (int count)
{
  int c;
